import React, { useState, useEffect } from 'react';
import { Link, useLocation, useNavigate } from 'react-router-dom';

const Navbar = ({ user, cartCount, wishlistCount, onLogout, openCart, openWishlist, openProfileModal, openMobDrawer, openNotifs }) => {
  const location = useLocation();
  const navigate = useNavigate();
  const [scrolled, setScrolled] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [searchOpen, setSearchQueryOpen] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      setScrolled(window.scrollY > 20);
    };
    window.addEventListener('scroll', handleScroll);
    return () => window.removeEventListener('scroll', handleScroll);
  }, []);

  const isActive = (path) => location.pathname === path;

  const handleSearch = (e) => {
    if (e.key === 'Enter') {
      navigate(`/shop?search=${searchQuery}`);
      setSearchQueryOpen(false);
    }
  };

  return (
    <nav id="main-nav" className={scrolled ? 'scrolled' : ''} style={{ display: 'block' }}>
      <div className="nav-in">
        <Link to="/" className="nav-logo">
          <img className="nav-logo-img" src="data:image/png;base64, " alt="DentalShark Logo" />
          <span className="logo-text">Dental<span className="teal-text">Shark</span></span>
        </Link>

        <div className="nav-links desktop-only">
          <Link to="/" className={`nl ${isActive('/') ? 'active' : ''}`}>HOME</Link>
          <Link to="/shop" className={`nl ${isActive('/shop') ? 'active' : ''}`}>SHOP</Link>
          <Link to="/ai" className={`nl ${isActive('/ai') ? 'active' : ''}`}>AI SCANNER</Link>
          <Link to="/engineers" className={`nl ${isActive('/engineers') ? 'active' : ''}`}>ENGINEERS</Link>
          <Link to="/about" className={`nl ${isActive('/about') ? 'active' : ''}`}>ABOUT</Link>
          <Link to="/contact" className={`nl ${isActive('/contact') ? 'active' : ''}`}>CONTACT</Link>
        </div>

        <div className="nav-actions">
          <div id="nav-search-desktop" className={`nav-search-wrap desktop-only ${searchOpen ? 'open' : ''}`}>
            <input 
              id="nsi" 
              className="nav-search-input" 
              placeholder="Search products..." 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onKeyDown={handleSearch}
              onFocus={() => setSearchQueryOpen(true)}
            />
            <button className="nav-btn" onClick={() => setSearchQueryOpen(!searchOpen)}>🔍</button>
          </div>

          <button className="nav-btn" onClick={openNotifs} title="Notifications">
            <span className="nav-icon">🔔</span>
            <span id="notif-badge" className="badge">0</span>
          </button>

          <button className="nav-btn desktop-only" onClick={openWishlist} title="Wishlist">
            <span className="nav-icon">❤️</span>
            {wishlistCount > 0 && <span id="wl-badge" className="badge show">{wishlistCount}</span>}
          </button>

          <button className="nav-btn" onClick={openCart} title="Your Cart">
            <span className="nav-icon">🛒</span>
            {cartCount > 0 && <span id="cart-badge" className="badge show">{cartCount}</span>}
          </button>

          <div id="nav-user-wrap">
            {user ? (
              <div className="user-chip" onClick={() => openProfileModal(user.id)}>
                <div className="user-av" style={{ background: user.color }}>{user.initials}</div>
                <span className="user-name">{user.name.split(" ").slice(-2).join(" ")}</span>
              </div>
            ) : (
              <button className="btn-signin" onClick={() => navigate('/login')}>SIGN IN</button>
            )}
          </div>

          <button className="nav-btn mob-only" onClick={openMobDrawer}>☰</button>
        </div>
      </div>
    </nav>
  );
};

export default Navbar;
